export const ADD_BURGER_INGREDIENTS:"ADD_BURGER_INGREDIENTS" = "ADD_BURGER_INGREDIENTS";
export const GET_COUNTER:"GET_COUNTER" = "GET_COUNTER";
export const ADD_CONSTRUCTOR_ELEMENT:"ADD_CONSTRUCTOR_ELEMENT" = "ADD_CONSTRUCTOR_ELEMENT";
export const REMOVE_CONSTRUCTOR_ELEMENT:"REMOVE_CONSTRUCTOR_ELEMENT" = "REMOVE_CONSTRUCTOR_ELEMENT";
export const GET_DETAILS:"GET_DETAILS" = "GET_DETAILS";
export const UPDATE_SELECTED_ITEMS_ORDER:"UPDATE_SELECTED_ITEMS_ORDER" = "UPDATE_SELECTED_ITEMS_ORDER";
export const GET_INGREDIENTS_REQUEST:"GET_INGREDIENTS_REQUEST" = "GET_INGREDIENTS_REQUEST";
export const GET_INGREDIENTS_SUCCESS:"GET_INGREDIENTS_SUCCESS" = "GET_INGREDIENTS_SUCCESS";
export const GET_INGREDIENTS_ERROR:"GET_INGREDIENTS_ERROR" = "GET_INGREDIENTS_ERROR";
export const GET_ORDER_REQUEST:"GET_ORDER_REQUEST" = "GET_ORDER_REQUEST";
export const GET_ORDER_SUCCESS:"GET_ORDER_SUCCESS" = "GET_ORDER_SUCCESS";
export const GET_ORDER_ERROR:"GET_ORDER_ERROR" = "GET_ORDER_ERROR";
export const REGISTER_PERSON_REQUEST:"REGISTER_PERSON_REQUEST" = "REGISTER_PERSON_REQUEST";
export const REGISTER_PERSON_SUCCESS:"REGISTER_PERSON_SUCCESS" = "REGISTER_PERSON_SUCCESS";
export const REGISTER_PERSON_ERROR:"REGISTER_PERSON_ERROR" = "REGISTER_PERSON_ERROR";
export const RESET_PASSWORD_REQUEST:"RESET_PASSWORD_REQUEST" = "RESET_PASSWORD_REQUEST";
export const RESET_PASSWORD_SUCCESS:"RESET_PASSWORD_SUCCESS" = "RESET_PASSWORD_SUCCESS";
export const RESET_PASSWORD_ERROR:"RESET_PASSWORD_ERROR" = "RESET_PASSWORD_ERROR";
export const LOGIN_REQUEST:"LOGIN_REQUEST" = "LOGIN_REQUEST";
export const LOGIN_SUCCESS:"LOGIN_SUCCESS" = "LOGIN_SUCCESS";
export const LOGIN_ERROR:"LOGIN_ERROR" = "LOGIN_ERROR";
export const LOGOUT_REQUEST:"LOGOUT_REQUEST" = "LOGOUT_REQUEST";
export const LOGOUT_SUCCESS:"LOGOUT_SUCCESS" = "LOGOUT_SUCCESS";
export const LOGOUT_ERROR:"LOGOUT_ERROR" = "LOGOUT_ERROR";
export const REFRESH_TOKEN_REQUEST:"REFRESH_TOKEN_REQUEST" = "REFRESH_TOKEN_REQUEST";
export const REFRESH_TOKEN_SUCCESS:"REFRESH_TOKEN_SUCCESS" = "REFRESH_TOKEN_SUCCESS";
export const REFRESH_TOKEN_ERROR:"REFRESH_TOKEN_ERROR" = "REFRESH_TOKEN_ERROR";
export const RESET_PASSWORD:"RESET_PASSWORD" = "RESET_PASSWORD";
export const REGISTER_USER:"REGISTER_USER" = "REGISTER_USER";
export const LOG_OUT:"LOG_OUT" = "LOG_OUT";
export const LOG_IN:"LOG_IN" = "LOG_IN";
export const SET_USER:"SET_USER" = "SET_USER";
export const GET_FEED:"GET_FEED" = "GET_FEED";
export const DELETE_DETAILS:"DELETE_DETAILS" = "DELETE_DETAILS";
export const DELETE_FEED:"DELETE_FEED" = "DELETE_FEED";
